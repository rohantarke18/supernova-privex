...
export function OrderForm({ address, deposits, revealed, balBusy, spotPrice, onRefreshBalances, onOrder }: Props) {
  ...
  async function submit(e: FormEvent) {
    ...
    const tid = toast.push({ kind: 'pending', title: `Placing ${side} order`, msg: 'Encrypting order…' });
    try {
  ...
  return (
    <form className="of" onSubmit={submit}>
      ...
      <div className="of__priv">🔒 Price &amp; size encrypted on-chain · solver never sees them</div>
    </form>
  );
}
